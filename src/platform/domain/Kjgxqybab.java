package platform.domain;
// default package

import java.util.HashSet;
import java.util.Set;


public class Kjgxqybab implements java.io.Serializable {

	// Fields

	  private Integer id;
	  private String year;
	  private String nf;
	  private String qymc;
	  private String nssbh;
	  private String ssly;
	  private String zgswjg;
	  private String zgzs;
	  private String yjrys;
	  private String dzrs;
	  private String ynzsr;
	  private String sr1;
	  private String sr2;
	  private String sr3;
	  private String hj;
	  private String xszzl;
	  private String zc1;
	  private String zc2;
	  private String zc3;
	  private String zczzl;
	private Set<Kjgxqybabzscq> kjgxqybabzscqs = new HashSet<Kjgxqybabzscq>();
	private Set<Kjgxqybabyfxm> kjgxqybabyfxms = new HashSet<Kjgxqybabyfxm>();
	private Set<Kjgxqybabgxcp> kjgxqybabgxcps = new HashSet<Kjgxqybabgxcp>();
	public Integer getId() {
		return id;
	}
	public void setId(Integer id) {
		this.id = id;
	}
	public String getYear() {
		return year;
	}
	public void setYear(String year) {
		this.year = year;
	}
	public String getNf() {
		return nf;
	}
	public void setNf(String nf) {
		this.nf = nf;
	}
	public String getQymc() {
		return qymc;
	}
	public void setQymc(String qymc) {
		this.qymc = qymc;
	}
	public String getNssbh() {
		return nssbh;
	}
	public void setNssbh(String nssbh) {
		this.nssbh = nssbh;
	}
	public String getSsly() {
		return ssly;
	}
	public void setSsly(String ssly) {
		this.ssly = ssly;
	}
	public String getZgswjg() {
		return zgswjg;
	}
	public void setZgswjg(String zgswjg) {
		this.zgswjg = zgswjg;
	}
	public String getZgzs() {
		return zgzs;
	}
	public void setZgzs(String zgzs) {
		this.zgzs = zgzs;
	}
	public String getYjrys() {
		return yjrys;
	}
	public void setYjrys(String yjrys) {
		this.yjrys = yjrys;
	}
	public String getDzrs() {
		return dzrs;
	}
	public void setDzrs(String dzrs) {
		this.dzrs = dzrs;
	}
	public String getYnzsr() {
		return ynzsr;
	}
	public void setYnzsr(String ynzsr) {
		this.ynzsr = ynzsr;
	}
	public String getSr1() {
		return sr1;
	}
	public void setSr1(String sr1) {
		this.sr1 = sr1;
	}
	public String getSr2() {
		return sr2;
	}
	public void setSr2(String sr2) {
		this.sr2 = sr2;
	}
	public String getSr3() {
		return sr3;
	}
	public void setSr3(String sr3) {
		this.sr3 = sr3;
	}
	public String getHj() {
		return hj;
	}
	public void setHj(String hj) {
		this.hj = hj;
	}
	public String getXszzl() {
		return xszzl;
	}
	public void setXszzl(String xszzl) {
		this.xszzl = xszzl;
	}
	public String getZc1() {
		return zc1;
	}
	public void setZc1(String zc1) {
		this.zc1 = zc1;
	}
	public String getZc2() {
		return zc2;
	}
	public void setZc2(String zc2) {
		this.zc2 = zc2;
	}
	public String getZc3() {
		return zc3;
	}
	public void setZc3(String zc3) {
		this.zc3 = zc3;
	}
	public String getZczzl() {
		return zczzl;
	}
	public void setZczzl(String zczzl) {
		this.zczzl = zczzl;
	}
	public Set<Kjgxqybabzscq> getKjgxqybabzscqs() {
		return kjgxqybabzscqs;
	}
	public void setKjgxqybabzscqs(Set<Kjgxqybabzscq> kjgxqybabzscqs) {
		this.kjgxqybabzscqs = kjgxqybabzscqs;
	}
	public Set<Kjgxqybabyfxm> getKjgxqybabyfxms() {
		return kjgxqybabyfxms;
	}
	public void setKjgxqybabyfxms(Set<Kjgxqybabyfxm> kjgxqybabyfxms) {
		this.kjgxqybabyfxms = kjgxqybabyfxms;
	}
	public Set<Kjgxqybabgxcp> getKjgxqybabgxcps() {
		return kjgxqybabgxcps;
	}
	public void setKjgxqybabgxcps(Set<Kjgxqybabgxcp> kjgxqybabgxcps) {
		this.kjgxqybabgxcps = kjgxqybabgxcps;
	}
	
	

	


}
