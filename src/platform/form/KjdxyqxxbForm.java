package platform.form;

import java.util.Date;

@SuppressWarnings({ "unused", "serial" })
public class KjdxyqxxbForm implements java.io.Serializable{
	  private String id;
	  private String yqbh;
	  private String flbm;
	  private String nbbh;
	  private String zwmc;
	  private String ywmc;
	  private String xhgg;
	  private String jszb;
	  private String csyjly;
	  private String zyyt;
	  private String zyfjgn;
	  private String rzqk;
	  private String sccs;
	  private String cdgb;
	  private String yqyz;
	  private String qyrq;
	  private String szdwmc;
	  private String sysmc;
	  private String afdz;
	  private String yzbm;
	  private String lxr;
	  private String dh;
	  private String dzyj;
	  private String sfgx;
	  private String yqzt;
	  private String kfjsap;
	  private String cksfbz;
	  private String fwtjnf;
	  private String nfwjs;
	  private String nfwsr;
	  private String zmyh;
	  private String yycg;
	  private String yqtp;
	  private String bz;
	public String getId() {
		return id;
	}
	public void setId(String id) {
		this.id = id;
	}
	public String getYqbh() {
		return yqbh;
	}
	public void setYqbh(String yqbh) {
		this.yqbh = yqbh;
	}
	public String getFlbm() {
		return flbm;
	}
	public void setFlbm(String flbm) {
		this.flbm = flbm;
	}
	public String getNbbh() {
		return nbbh;
	}
	public void setNbbh(String nbbh) {
		this.nbbh = nbbh;
	}
	public String getZwmc() {
		return zwmc;
	}
	public void setZwmc(String zwmc) {
		this.zwmc = zwmc;
	}
	public String getYwmc() {
		return ywmc;
	}
	public void setYwmc(String ywmc) {
		this.ywmc = ywmc;
	}
	public String getXhgg() {
		return xhgg;
	}
	public void setXhgg(String xhgg) {
		this.xhgg = xhgg;
	}
	public String getJszb() {
		return jszb;
	}
	public void setJszb(String jszb) {
		this.jszb = jszb;
	}
	public String getCsyjly() {
		return csyjly;
	}
	public void setCsyjly(String csyjly) {
		this.csyjly = csyjly;
	}
	public String getZyyt() {
		return zyyt;
	}
	public void setZyyt(String zyyt) {
		this.zyyt = zyyt;
	}
	public String getZyfjgn() {
		return zyfjgn;
	}
	public void setZyfjgn(String zyfjgn) {
		this.zyfjgn = zyfjgn;
	}
	public String getRzqk() {
		return rzqk;
	}
	public void setRzqk(String rzqk) {
		this.rzqk = rzqk;
	}
	public String getSccs() {
		return sccs;
	}
	public void setSccs(String sccs) {
		this.sccs = sccs;
	}
	public String getCdgb() {
		return cdgb;
	}
	public void setCdgb(String cdgb) {
		this.cdgb = cdgb;
	}
	public String getYqyz() {
		return yqyz;
	}
	public void setYqyz(String yqyz) {
		this.yqyz = yqyz;
	}
	public String getQyrq() {
		return qyrq;
	}
	public void setQyrq(String qyrq) {
		this.qyrq = qyrq;
	}
	public String getSzdwmc() {
		return szdwmc;
	}
	public void setSzdwmc(String szdwmc) {
		this.szdwmc = szdwmc;
	}
	public String getSysmc() {
		return sysmc;
	}
	public void setSysmc(String sysmc) {
		this.sysmc = sysmc;
	}
	public String getAfdz() {
		return afdz;
	}
	public void setAfdz(String afdz) {
		this.afdz = afdz;
	}
	public String getYzbm() {
		return yzbm;
	}
	public void setYzbm(String yzbm) {
		this.yzbm = yzbm;
	}
	public String getLxr() {
		return lxr;
	}
	public void setLxr(String lxr) {
		this.lxr = lxr;
	}
	public String getDh() {
		return dh;
	}
	public void setDh(String dh) {
		this.dh = dh;
	}
	public String getDzyj() {
		return dzyj;
	}
	public void setDzyj(String dzyj) {
		this.dzyj = dzyj;
	}
	public String getSfgx() {
		return sfgx;
	}
	public void setSfgx(String sfgx) {
		this.sfgx = sfgx;
	}
	public String getYqzt() {
		return yqzt;
	}
	public void setYqzt(String yqzt) {
		this.yqzt = yqzt;
	}
	public String getKfjsap() {
		return kfjsap;
	}
	public void setKfjsap(String kfjsap) {
		this.kfjsap = kfjsap;
	}
	public String getCksfbz() {
		return cksfbz;
	}
	public void setCksfbz(String cksfbz) {
		this.cksfbz = cksfbz;
	}
	public String getFwtjnf() {
		return fwtjnf;
	}
	public void setFwtjnf(String fwtjnf) {
		this.fwtjnf = fwtjnf;
	}
	public String getNfwjs() {
		return nfwjs;
	}
	public void setNfwjs(String nfwjs) {
		this.nfwjs = nfwjs;
	}
	public String getNfwsr() {
		return nfwsr;
	}
	public void setNfwsr(String nfwsr) {
		this.nfwsr = nfwsr;
	}
	public String getZmyh() {
		return zmyh;
	}
	public void setZmyh(String zmyh) {
		this.zmyh = zmyh;
	}
	public String getYycg() {
		return yycg;
	}
	public void setYycg(String yycg) {
		this.yycg = yycg;
	}
	public String getYqtp() {
		return yqtp;
	}
	public void setYqtp(String yqtp) {
		this.yqtp = yqtp;
	}
	public String getBz() {
		return bz;
	}
	public void setBz(String bz) {
		this.bz = bz;
	}
	  
}