...
//加载公文列表
function listDoc() {
	var actionPath = basePath + '/system/ProfessorAction_list.action';
	 $('#dg').datagrid({
            title : '专家综合查询表',
            //width : 1200,
            height: 400,
...
function editDoc() {
	...
	$("#gzbm").val(doc.gzbm);
	$("#zw").val(doc.zw);
	$("#cgmcZzq").val(doc.cgmcZzq);
	$("#cgmcQt").val(doc.cgmcQt);
	$("#cgmcDj").val(doc.cgmcDj);
	$("#cgmcZl").val(doc.cgmcZl);
...
function dealSave() {
	...
	var actionAdd = basePath + '/system/ProfessorAction_add.action';
	var actionUpdate = basePath + '/system/ProfessorAction_update.action';
...
function deleteDoc() {
	...
	$.messager.confirm('确认', '真的要删除选中的记录吗？', function(r) {
		if (r) {
			var actionPath = basePath + '/system/ProfessorAction_delete.action?id=';
	             ...
	             $.messager.alert('提示','请选择上传文件！','info');   
	              ...
	              if(d1==".xls"){  
	            	...
	            	//document.getElementById("questionTypesManage").submit();     
	            		...
	            		var showimport = basePath + '/system/ProfessorAction_showimport.action';
...
function ShowExport(){
  ...
  { 
	...
	var showimport = basePath + '/system/ProfessorAction_showexport.action';
