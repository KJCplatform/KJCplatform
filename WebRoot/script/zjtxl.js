...
//加载公文列表
function listDoc() {
	var actionPath = basePath + '/system/ZjtxlAction_list.action';
	 $('#dg').datagrid({
            title : '专家通讯录',
            width : 1200,
            height: 400,
...
function editDoc() {
	...
	$("#bm").val(doc.bm);	
	$("#zw").val(doc.zw);
	$("#lxfszj").val(doc.lxfszj);
	$("#lxfssj").val(doc.lxfssj);
	// 显示编辑页面
	showEditForm();
}
function dealSave() {
	...
	var actionAdd = basePath + '/system/ZjtxlAction_add.action';
	var actionUpdate = basePath + '/system/ZjtxlAction_update.action';
...
function deleteDoc() {
	...
	$.messager.confirm('确认', '真的要删除选中的记录吗？', function(r) {
		if (r) {
			var actionPath = basePath + '/system/ZjtxlAction_delete.action?id=';
