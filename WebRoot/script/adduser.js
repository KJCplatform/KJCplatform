...
//加载公文列表
function listDoc() {
	var actionPath = basePath + '/system/AdduserAction_list.action';
	 $('#dg').datagrid({
            title : '添加用户',
            width : 1200,
            height: 400,
...
function dealSave() {
	...
	var actionAdd = basePath + '/system/AdduserAction_add.action';
	var actionUpdate = basePath + '/system/AdduserAction_update.action';
...
function deleteDoc() {
	...
	$.messager.confirm('确认', '真的要删除选中的记录吗？', function(r) {
		if (r) {
			var actionPath = basePath + '/system/AdduserAction_delete.action?id=';
