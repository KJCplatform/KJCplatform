...
function print1() {
	...
	// LODOP.ADD_PRINT_TABLE(50,14,500,500,strHTML);
	/*
	 * <style media="print"> //.Noprint { DISPLAY: none;} strHTML.PageNext {
	 * PAGE-BREAK-AFTER: always } </style>
	 */
	LODOP.ADD_PRINT_HTM(0, 0, "1%", "1%", strHTML);
	LODOP.NewPage();
	LODOP.SET_PRINT_STYLEA(0, "FontSize", 15);
	LODOP.SET_PRINT_STYLEA(0, "ItemType", 4);
	LODOP.SET_PRINT_STYLEA(0, "Horient", 3);
...
function exportExcel() {
	...
	var strFormHtml = strBodyStyle + "<body>"
			+ document.getElementByTagName("html")[0].innerHTML + "</body>";
...
function CreatePrintPage() {
	...
	if (document.getElementById("content") != null)
		var strFormHtml = strBodyStyle + "<body>"
				+ document.getElementById("dg").innerHTML + "</body>";
	// 打印初始化，页面距离顶部10px，距离左边10px，宽754px，高453px，给打印设置个标题
	LODOP.PRINT_INITA(10, 10, 754, 453, "打印控件操作");
	// 设置打印页面属性：2：表示横向打印，0：定义纸张宽度，为0表示无效设置，A4：设置纸张为A4
	LODOP.SET_PRINT_PAGESIZE(2, 0, 0, "A4");
...
function ShowExport() {
	...
	var showimport = basePath + '/system/Wqwqxkzxq0Action_showexport.action';
