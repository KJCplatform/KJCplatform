...
//加载公文列表
function listDoc() {
	var actionPath = basePath + '/system/Kjjszjcjb4Action_list.action';
	 $('#dg').datagrid({
            title : '许可专家',
            width : 1500,
            height: 400,
...
function dealSave() {
	...
	var actionAdd = basePath + '/system/Kjjszjcjb4Action_add.action';
	var actionUpdate = basePath + '/system/Kjjszjcjb4Action_update.action';
...
function deleteDoc() {
	...
	$.messager.confirm('确认', '真的要删除选中的记录吗？', function(r) {
		if (r) {
			var actionPath = basePath + '/system/Kjjszjcjb4Action_delete.action?id=';
	...
	if (fileName == "") {
		...
		if (d1 == ".xls") {
			...
			var showimport = basePath
					+ '/system/Kjjszjcjb4Action_showimport.action';
...
function ShowExport() {
	...
	var showimport = basePath + '/system/Kjjszjcjb4Action_showexport.action';
