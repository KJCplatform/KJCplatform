...
//加载公文列表
function listDoc() {
	var actionPath = basePath + '/system/Wqwqxkzxq4Action_list.action';
	 $('#dg').datagrid({
            title : '申请增加的武器装备科研生产许可专业产品',
            width : 1200,
            height: 400,
            //fit: true,
...
function dealSave() {
	...
	var actionAdd = basePath + '/system/Wqwqxkzxq4Action_add.action';
	var actionUpdate = basePath + '/system/Wqwqxkzxq4Action_update.action';
...
function deleteDoc() {
	...
	$.messager.confirm('确认', '真的要删除选中的记录吗？', function(r) {
		if (r) {
			var actionPath = basePath + '/system/Wqwqxkzxq4Action_delete.action?id=';
	...
	if (fileName == "") {
		...
		if (d1 == ".xls") {
			...
			var showimport = basePath
					+ '/system/Wqwqxkzxq4Action_showimport.action';
...
function ShowExport() {
	...
	var showimport = basePath + '/system/Wqwqxkzxq4Action_showexport.action';
