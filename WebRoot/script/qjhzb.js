...
//	});
	...
	$("#jlzy").validatebox({
		required : true,
		missingMessage : '所属计量专业不能为空'
	});

});
//加载公文列表
function listDoc() {
	var actionPath = basePath + '/system/JljlqjhzbAction_list.action';
	 $('#dg').datagrid({
...
function dealSave() {
	...
	var actionAdd = basePath + '/system/JljlqjhzbAction_add.action';
	var actionUpdate = basePath + '/system/JljlqjhzbAction_update.action';
...
function deleteDoc() {
	...
	$.messager.confirm('确认', '真的要删除选中的记录吗？', function(r) {
		if (r) {
			var actionPath = basePath + '/system/JljlqjhzbAction_delete.action?id=';
	...
	if (fileName == "") {
		...
		if (d1 == ".xls") {
			...
			var showimport = basePath
					+ '/system/JljlqjhzbAction_showimport.action';
...
function ShowExport() {
	...
	var showimport = basePath + '/system/JljlqjhzbAction_showexport.action';
