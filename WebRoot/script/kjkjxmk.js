...
function editDoc() {
	...
	$("#jsgmnr").val(doc.jsgmnr);
	$("#xmjszq").val(doc.xmjszq);
	$("#ymjd").val(doc.ymjd);
	$("#ztz").val(doc.ztz);
	$("#ywctz").val(doc.ywctz);
	$("#bntz").val(doc.bntz);
...
function dealSave() {
	...
	var actionAdd = basePath + '/system/KjkjxmkAction_add.action';
	var actionUpdate = basePath + '/system/KjkjxmkAction_update.action';
...
function deleteDoc() {
	...
	$.messager.confirm('确认', '真的要删除选中的记录吗？', function(r) {
		if (r) {
			var actionPath = basePath + '/system/KjkjxmkAction_delete.action?id=';
