...
function listDoc() {
	var actionPath = basePath + '/system/KjdxyqxxbAction_list.action';
	 $('#dg').datagrid({
            title : '大型仪器信息表',
            width : 1200,
            height: 400,
            ...
            sortName : 'qyrq',//当数据表格初始化时以哪一列来排序
...
function editDoc() {
	...
	$("#xhgg").val(doc.xhgg);
	$("#jszb").val(doc.jszb);
	$("#csyjly").val(doc.csyjly);
	$("#zyyt").val(doc.zyyt);
	$("#zyfjgn").val(doc.zyfjgn);
	$("#rzqk").val(doc.rzqk);
	$("#sccs").val(doc.sccs);
	$("#cdgb").val(doc.cdgb);
	$("#yqyz").val(doc.yqyz);
	...
	$("#sfgx").val(doc.sfgx);
	$("#yqzt").val(doc.yqzt);
	$("#kfjsap").val(doc.kfjsap);
	$("#cksfbz").val(doc.cksfbz);
	$("#fwtjnf").val(doc.fwtjnf);
	$("#nfwjs").val(doc.nfwjs);
	$("#nfwsr").val(doc.nfwsr);
	$("#zmyh").val(doc.zmyh);
	$("#yycg").val(doc.yycg);
...
function dealSave() {
	...
	var actionAdd = basePath + '/system/KjdxyqxxbAction_add.action';
	var actionUpdate = basePath + '/system/KjdxyqxxbAction_update.action';
...
function deleteDoc() {
	...
	$.messager.confirm('确认', '真的要删除选中的记录吗？', function(r) {
		if (r) {
			var actionPath = basePath + '/system/KjdxyqxxbAction_delete.action?id=';
